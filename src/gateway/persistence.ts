...
export class PersistenceManager {
    ...
    // --- Messages ---
    async storeMessage(chatId: string | number, sender: string, text: string) {
        this.db.run(
            "INSERT INTO messages (chat_id, sender, text) VALUES (?, ?, ?)",
            [String(chatId), sender, text]
        );
    }

    async getHistory(chatId: string | number, limit: number = 50): Promise<DBMessage[]> {
        return this.db.query("SELECT * FROM messages WHERE chat_id = ? ORDER BY id DESC LIMIT ?")
            .all(String(chatId), limit) as DBMessage[];
    }
    ...
    async getSession(chatId: string | number): Promise<string | null> {
        const result = this.db.query("SELECT instance_name FROM sessions WHERE chat_id = ?")
    ...
    async saveTask(task: any) {
        this.db.run(
            ...
            [task.id, task.instance_name, task.chat_id, task.prompt, task.schedule_type, task.schedule_value, task.next_run, task.status]
        );
    }
