 ...
 * The InProcessEngine uses it as a thin adapter to fulfill the IExecutionEngine interface.
 *
 ...
 * Exported so InProcessEngine (and others) can reuse instead of duplicating.
 */
...
export class EmbeddedAgent {
	...
	constructor(config: EmbeddedAgentConfig) {
		...
		// Configure via setter methods (CORRECTION 3: use setters, not constructor)
	...
	async initialize(): Promise<void> {
		...
		// Validate API key is available (H2: fail early with clear error)
	 ...
	 *
	 * CORRECTION 1: Agent.prompt() returns Promise<void>, not a result.
	 * Results are collected via subscribe() + EventCollector.
	 *
	...
	async prompt(message: string, options?: PromptOptions): Promise<AgentResult> {
		// H1: Guard against concurrent prompt() calls
		...
		// Set up event collector with maxIterations guard (GAP 1)
		...
		// Subscribe to events BEFORE calling prompt (CORRECTION 1)
	 ...
	 * CORRECTION 5: Use agent.state.messages, not getHistory()
	 */
	 ...
	 * CORRECTION 4: clearHistory() -> clearMessages()
	 */
	clearMessages(): void {
	 ...
	 * Mirrors the existing buildModel() from InProcessEngine.
	 */
