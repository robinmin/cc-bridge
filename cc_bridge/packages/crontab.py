...
It is primarily used by the setup wizard to configure periodic health checks.
"""
...
class CrontabManager:
    ...
    All modifications are marked with CC-BRIDGE comments for easy identification.

    Attributes:
        CC_BRIDGE_MARKER: Start marker for CC-BRIDGE crontab entries
        CC_BRIDGE_MARKER_END: End marker for CC-BRIDGE crontab entries

    Examples:
        >>> manager = CrontabManager()
        >>> manager.add_entry("*/5 * * * * /path/to/cc-bridge health")
        True
        >>> manager.has_entries()
        True
    ...
    def _get_current_crontab(self) -> list[str]:
        ...
        Returns:
            List of crontab lines (empty if crontab doesn't exist or on error)
        """
        try:
    ...
    def _backup_crontab(self) -> list[str]:
        ...
        The backup is saved to ~/.claude/bridge/crontab.backup

        Returns:
            List of crontab lines that were backed up
        """
    ...
    def _validate_entry(self, entry: str) -> bool:
        ...
        - At least 6 fields (5 time fields + command)
        - Time fields are valid cron patterns (numbers, *, or */n)
        ...
        Examples:
            >>> manager._validate_entry("*/5 * * * * /usr/bin/true")
            True
            >>> manager._validate_entry("invalid")
            False
        """
        parts = entry.split()
    ...
    def add_entry(self, entry: str, validate: bool = True) -> bool:
        """
        Add entry to crontab with CC-BRIDGE markers.

        The entry is wrapped with CC-BRIDGE marker comments for easy identification
        and removal. A backup is created before modification.

        Args:
            entry: Crontab entry to add
            validate: Whether to validate entry format (default: True)
        ...
        Examples:
            >>> manager = CrontabManager()
            >>> manager.add_entry("*/5 * * * * cc-bridge health")
            True
        """
    ...
    def remove_entry(self) -> bool:
        ...
        Removes all lines between the CC-BRIDGE markers (inclusive).
        This effectively removes the entry and its marker comments.
    ...
    def has_entries(self) -> bool:
        ...
        Returns:
            True if CC-BRIDGE marker found, False otherwise
        """
        crontab_lines = self._get_current_crontab()
    ...
    def restore_backup(self) -> bool:
        ...
        Reads the backup file from ~/.claude/bridge/crontab.backup
        and restores it to the current crontab.

        Returns:
            True if successful, False otherwise (including if backup doesn't exist)
        """
        backup_file = Path.home() / ".claude" / "bridge" / "crontab.backup"
