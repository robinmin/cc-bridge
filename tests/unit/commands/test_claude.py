...
from cc_bridge.commands.claude import app
...
class TestValidateWorkingDirectory:
    ...
    def test_validate_existing_directory(self, tmp_path: Path):
        ...
        is_valid, result = validate_working_directory(str(tmp_path))
    ...
    def test_validate_creates_directory(self, tmp_path: Path):
        ...
        is_valid, result = validate_working_directory(str(new_dir))
    ...
    def test_validate_expands_tilde(self):
        ...
        is_valid, result = validate_working_directory("~/test")
        assert is_valid is True
    ...
    def test_validate_rejects_file(self, tmp_path: Path):
        ...
        is_valid, result = validate_working_directory(str(test_file))
...
class TestGetSessionName:
    ...
    def test_session_name_format(self):
        ...
        assert get_session_name("test") == "claude-test"
        assert get_session_name("my-project") == "claude-my-project"
        assert get_session_name("instance_1") == "claude-instance_1"
...
class TestClaudeCommand:
    ...
    def mock_tmux_available(self):
        ...
        with patch("cc_bridge.core.instance_lifecycle.is_tmux_available") as mock:
            mock.return_value = True
            yield mock

    def test_start_new_instance(
        self, mock_instance_manager, mock_tmux_available, tmp_path: Path
    ):
        ...
        with patch("cc_bridge.core.instance_operations.subprocess.run") as mock_run:
    ...
    def test_start_existing_instance_fails(
        self, mock_instance_manager, mock_tmux_available
    ):
    ...
    def test_start_without_tmux_fails(self, mock_instance_manager):
        """Should fail when tmux is not available."""

        runner = CliRunner()
        # Mock that no instance exists
        mock_instance_manager.get_instance.return_value = None

        with patch("cc_bridge.core.instance_operations.is_tmux_available") as mock:
