...
from cc_bridge.core.webhook import (
    get_instance_manager_dep,
    get_telegram_client_dep,
)
from cc_bridge.core.webhook import middleware
...
@pytest.mark.asyncio
@patch("cc_bridge.core.webhook.handlers.get_config")
...
async def test_webhook_rate_limit_returns_429():
    ...
    # Mock rate limiter that always returns False (rate limited)
    mock_limiter = MagicMock()
    mock_limiter.is_allowed = AsyncMock(return_value=False)
    mock_limiter.get_retry_after = AsyncMock(return_value=60)

    app.dependency_overrides[middleware.get_rate_limiter] = lambda: mock_limiter
    ...
    try:
        ...
        # Clear override
        if middleware.get_rate_limiter in app.dependency_overrides:
            del app.dependency_overrides[middleware.get_rate_limiter]
...
async def test_webhook_message_too_long_returns_400():
    ...
    # Mock mocks to prevent side effects
    mock_manager = MagicMock()
    mock_manager.list_instances.return_value = []
